...
from agents.formatting import bar_granularity, format_market_state
...
Given recent closing prices and a realized-volatility estimate, classify the current \
...
bar-to-bar swings are large regardless of direction; "range_bound" when price oscillates without a \
...
def _realized_volatility(bars: pd.DataFrame, lookback: int = 20) -> float:
    """Realized volatility (stdev of per-bar returns, as a percent) over the
    lookback window -- a cheap, deterministic signal to ground the LLM's
    classification instead of asking it to eyeball a list of prices."""
...
def build_prompt(symbol: str, bars: pd.DataFrame, lookback: int = 20) -> str:
    ...
    unit = "bar" if bar_granularity(bars)[0] else "day"
    return (
        f"{format_market_state(symbol, bars, lookback)}\n\n"
        f"{lookback}-{unit} realized volatility (stdev of per-{unit} returns): {vol:.2f}%\n"
